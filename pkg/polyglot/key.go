...
const (
	polyRandomPieceOffset     = 0
	polyRandomCastleOffset    = 768
	polyRandomEnPassantOffset = 772
	polyRandomTurnOffset      = 780
)

// keyHash returns a hash (uint64)
//
...
func keyHash(position *chess.Position) (hash uint64) {
	hash = hash ^ pieceHash(position)
	hash = hash ^ enPassantHash(position)
	hash = hash ^ turnHash(position)
	hash = hash ^ castleHash(position)
	return
}

// pieceHash returns a hash (uint64)
//
// The hash is the exclusive or of entries from randomPiece, one for each
// piece of the board. The offset of each entry is computed as follow:
//
//	offset = 64 * pieceKindOffsets[piece] + 8 * rank + file
func pieceHash(position *chess.Position) (hash uint64) {
	for square, piece := range position.Board().SquareMap() {
		if piece != chess.NoPiece {
			offset := 64*pieceKindOffsets[piece] + 8*int(square.Rank()) + int(square.File())
			hash = hash ^ polyRandom[offset]
		}
	}
	return
}

// pieceKindOffsets holds the index offsets of each piece kinds.
var pieceKindOffsets = map[chess.Piece]int{
	chess.NoPiece:     0,
	chess.BlackPawn:   0,
	chess.WhitePawn:   1,
	chess.BlackKnight: 2,
...
//
// The hash is the the exclusive or of entries from randomCastle.
// Entries are selected depending on which castle rights are available.
// If the position has no castle rights, 0 is returned.
//
//	white king side castle   0
//	white queen side castle  1
//	black king side castle   2
//	black queen side castle  3
func castleHash(position *chess.Position) (hash uint64) {
	colors := []chess.Color{chess.White, chess.Black}
	sides := []chess.Side{chess.KingSide, chess.QueenSide}
	for c, color := range colors {
		for s, side := range sides {
			offset := 2*c + s
			if position.CastleRights().CanCastle(color, side) {
				hash = hash ^ polyRandom[polyRandomCastleOffset+offset]
			}
		}
	}
	return
...
//
// If the opponent has performed a double pawn push and there is a pawn next
...
// 0 is returned.
func enPassantHash(position *chess.Position) (hash uint64) {
	square := position.EnPassantSquare()
	if square == chess.NoSquare || !hasPawnForCapture(position) {
		return
	}
	return polyRandom[polyRandomEnPassantOffset+int(square.File())]
}

// hasPawnForCapture checks that there exists an pawn that can capture the en
// passant pawn. If there is no en passant pawn, false is returned.
func hasPawnForCapture(position *chess.Position) bool {
	epSquare := position.EnPassantSquare()
	if epSquare == chess.NoSquare {
		return false
	}

	pawn, offset := chess.WhitePawn, chess.Rank(-1)
	if position.Turn() == chess.Black {
		pawn, offset = chess.BlackPawn, 1
	}
	...
	switch epFile {
	case chess.FileA:
		return position.Board().Piece(rightSquare) == pawn
	case chess.FileH:
		return position.Board().Piece(leftSquare) == pawn
	default:
		return position.Board().Piece(rightSquare) == pawn || position.Board().Piece(leftSquare) == pawn
	}
}

// turnHash returns a hash (uint64), which is the sole entry of randomTurn if
// white is to move and 0 otherwise
func turnHash(position *chess.Position) (hash uint64) {
...
// polyRandom contains four sub arrays
//
